...
## 📸 예시 화면 (Example Screenshot)

<div align="center">
  <img src="preview.png" alt="ETS2-Overlay Preview" width="800">
</div>
...
#### 옵션 1: 릴리즈 파일 다운로드 (권장)
1. [Releases](https://github.com/Likeit11/ETS2-Overlay/releases) 페이지에서 가장 최신 버전의 `Truck-Overlay.zip`을 다운로드합니다.
2. 원하는 폴더에 압축을 해제합니다.
3. 해당 폴더에서 터미널(CMD/PowerShell)을 열고 아래 명령어를 입력하여 필요한 라이브러리를 설치합니다:
   ```bash
   npm install
   ```
...
#### 옵션 2: 소스 코드 클론 (개발자용)
```bash
git clone https://github.com/Likeit11/ETS2-Overlay.git
cd ETS2-Overlay
npm install
```
...
1. `ets2-telemetry-server/server/Ets2Telemetry.exe`를 실행합니다.
2. **Install** 버튼을 클릭하여 텔레메트리 플러그인 DLL을 게임 디렉토리에 설치합니다.
3. 설치가 완료되면 서버를 닫아도 됩니다. (이후에는 배치 파일이 자동 실행합니다.)

> [!IMPORTANT]
> 만약 제대로 설치가 안될 경우 **관리자 권한**으로 실행하여 설치합니다. 이후 실행부터는 일반 권한으로도 동작합니다.
